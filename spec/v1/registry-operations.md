...
4. **Name + version match path.** `pack.json` `name` and `version` fields MUST match the URL path `/v1/packs/{name}/-/{version}.tgz`. Mismatch → `400 manifest_path_mismatch`.
5. **Version uniqueness.** `(name, version)` MUST NOT already exist. Conflict → `409 conflict`. Semver is immutable per the npm convention; republishing requires a new version.
