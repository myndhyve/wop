...
> **Packages not yet published.** The published-package names (`@myndhyve/wop`, `@myndhyve/wop-conformance`, `wop-client`, `github.com/myndhyve/wop/sdk/go`) are reserved but the v1.0.0 release is still in progress. Until then, install from source by cloning this repo. Tracking issue: see the project ROADMAP.
